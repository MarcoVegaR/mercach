...
export default function ChargesByStatusDonut() {
    const { data, isLoading, isError, refetch } = useQuery<ApiResponse>({
        queryKey: ['dashboard', 'charges', 'by-status'],
        staleTime: 300_000,
        queryFn: async () => {
            const res = await fetch('/api/dashboard/charges/by-status', { headers: { Accept: 'application/json' } });
            if (!res.ok) throw new Error('Failed to load charges by status');
    ...
    return (
        <Card className="flex flex-col">
            <CardHeader className="items-center pb-0">
                <CardTitle>Cargos por estatus</CardTitle>
                <CardDescription>Emitidos, parciales, cancelados</CardDescription>
            </CardHeader>
