...
export default function FormPage(props: PageProps) {
    ...
    // Track unsaved changes to avoid submitting when nothing changed
    const initialData = {
        code: initial.code ?? '',
        name: initial.name ?? '',
        is_active: Boolean(initial.is_active ?? true),
    ...
    function handleCancel() {
        router.visit('/catalogs/concessionaire-type', { preserveScroll: true });
    }
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (mode === 'create') {
            clearUnsavedChanges();
            form.post(route('catalogs.concessionaire-type.store'));
        } else {
            const id = initial.id;
            ...
            clearUnsavedChanges();
            form.put(route('catalogs.concessionaire-type.update', id));
        }
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={mode === 'edit' ? 'Editar Tipo de concesionario' : 'Crear Tipo de concesionario'} />
            <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
                <div className="py-8">
                    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
                        <h1 className="mb-4 text-2xl font-bold text-gray-900 dark:text-gray-100">
                            {mode === 'edit' ? 'Editar' : 'Crear'} Tipo de concesionario
                        </h1>
