...
function ActionsCell({ row }: { row: Row }) {
    ...
    const canUpdate = !!auth?.can?.['catalogs.phone-area-code.update'];
    const canDelete = !!auth?.can?.['catalogs.phone-area-code.delete'];
    const canSetActive = !!auth?.can?.['catalogs.phone-area-code.setActive'];
    ...
    return (
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                        <Link href={`/catalogs/phone-area-code/${row.id}`} className="cursor-pointer">
                    ...
                    {canUpdate && (
                        <DropdownMenuItem asChild>
                            <Link href={`/catalogs/phone-area-code/${row.id}/edit`} className="cursor-pointer">
            ...
            <ConfirmAlert
                ...
                description={`¿Está seguro de eliminar el registro "${String(row.code ?? row.id)}"? Esta acción no se puede deshacer.`}
                confirmLabel="Eliminar"
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.delete(`/catalogs/phone-area-code/${row.id}`, {
                            preserveState: false,
                            preserveScroll: true,
                            onSuccess: () => resolve(),
            ...
            <ConfirmAlert
                ...
                description={`¿Está seguro de ${isActive ? 'desactivar' : 'activar'} el registro "${String(row.code ?? row.id)}"?`}
                confirmLabel={isActive ? 'Desactivar' : 'Activar'}
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.patch(
                            `/catalogs/phone-area-code/${row.id}/active`,
                            { active: !isActive },
                            {
                                preserveState: false,
                                preserveScroll: true,
