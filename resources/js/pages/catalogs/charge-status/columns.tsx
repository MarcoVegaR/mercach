...
function ActionsCell({ row }: { row: Row }) {
    ...
    const canUpdate = !!auth?.can?.['catalogs.charge-status.update'];
    const canDelete = !!auth?.can?.['catalogs.charge-status.delete'];
    const canSetActive = !!auth?.can?.['catalogs.charge-status.setActive'];
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                        <Link href={`/catalogs/charge-status/${row.id}`} className="cursor-pointer">
                    ...
                    {canUpdate && (
                        <DropdownMenuItem asChild>
                            <Link href={`/catalogs/charge-status/${row.id}/edit`} className="cursor-pointer">
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.delete(`/catalogs/charge-status/${row.id}`, {
                            preserveState: false,
                            preserveScroll: true,
                            onSuccess: () => resolve(),
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.patch(
                            `/catalogs/charge-status/${row.id}/active`,
                            { active: !isActive },
                            {
                                preserveState: false,
                                preserveScroll: true,
