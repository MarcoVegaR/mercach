...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('charge-status_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.charge-status.create'] || false,
        canEdit: auth?.can?.['catalogs.charge-status.update'] || false,
        canDelete: auth?.can?.['catalogs.charge-status.delete'] || false,
        canExport: auth?.can?.['catalogs.charge-status.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.charge-status.delete'] || false,
        canSetActive: auth?.can?.['catalogs.charge-status.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.charge-status.setActive'] || false,
    };
    ...
    const reloadData = React.useCallback(() => {
        ...
        router.get('/catalogs/charge-status', params, {
            preserveState: true,
            preserveScroll: true,
        });
    ...
    const breadcrumbs = [
        ...
        { title: 'Estados de cargo', href: '/catalogs/charge-status' },
    ];
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/charge-status/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting],
    );
                ...
                <div className="py-8">
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <div className="mb-8">
                            <IndexHeaderHero
                                icon={ClipboardCheck}
                                title="Estados de cargo"
                                description="Catálogo de estados posibles de los cargos (pendiente, pagado, etc.)."
                                actions={
                                    permissions.canCreate ? (
                                        <Link href="/catalogs/charge-status/create">
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Estados de cargo</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Estado de cargo Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('charge-status_table_density', d);
                                    }}
