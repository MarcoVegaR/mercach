...
export default function ShowPage() {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Motivo de traspaso de deuda: ${String((item as any).name ?? (item as any).code ?? (item as any).id)}`} />

            <ShowLayout
                header={
                    <div className="flex items-center gap-4">
                        <Link href="/catalogs/debt-transfer-reason" className="text-muted-foreground hover:text-foreground transition-colors">
                ...
                actions={
                    <div className="flex gap-2">
                        {hasEditRoute && (
                            <Button onClick={() => router.visit(`/catalogs/debt-transfer-reason/${item.id}/edit`)}>
                        ...
                        <ConfirmAlert
                            ...
                            onConfirm={async () => {
                                await new Promise<void>((resolve, reject) => {
                                    router.delete(`/catalogs/debt-transfer-reason/${item.id}`, {
                                        preserveState: false,
                                        preserveScroll: true,
                                        onSuccess: () => {
                                            resolve();
                                            router.visit('/catalogs/debt-transfer-reason');
                                        },
