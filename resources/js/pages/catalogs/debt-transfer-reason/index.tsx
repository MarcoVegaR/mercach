...
import { Database, Plus, Receipt } from 'lucide-react';
...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('debt-transfer-reason_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.debt-transfer-reason.create'] || false,
        canEdit: auth?.can?.['catalogs.debt-transfer-reason.update'] || false,
        canDelete: auth?.can?.['catalogs.debt-transfer-reason.delete'] || false,
        canExport: auth?.can?.['catalogs.debt-transfer-reason.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.debt-transfer-reason.delete'] || false,
        canSetActive: auth?.can?.['catalogs.debt-transfer-reason.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.debt-transfer-reason.setActive'] || false,
    };
    ...
    const reloadData = React.useCallback(() => {
        ...
        router.get('/catalogs/debt-transfer-reason', params, {
            preserveState: true,
            preserveScroll: true,
        });
    ...
    const breadcrumbs = [
        ...
        { title: 'Motivos de traspaso de deuda', href: '/catalogs/debt-transfer-reason' },
    ];
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/debt-transfer-reason/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting],
    );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Motivos de traspaso de deuda" />
                ...
                <div className="py-8">
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <div className="mb-8">
                            <IndexHeaderHero
                                icon={Receipt}
                                title="Motivos de traspaso de deuda"
                                description="Catálogo de motivos para traspasos de deuda."
                                actions={
                                    permissions.canCreate ? (
                                        <Link href="/catalogs/debt-transfer-reason/create">
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Motivos de traspaso de deuda</p>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">
                                                Motivo de traspaso de deuda Activos
                                            </p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('debt-transfer-reason_table_density', d);
                                    }}
                        ...
                        <ConfirmAlert
                            ...
                            onConfirm={async () => {
                                ...
                                await new Promise<void>((resolve, reject) => {
                                    router.post(
                                        '/catalogs/debt-transfer-reason/bulk',
                                        { action: 'delete', ids },
                                        {
                                            preserveState: false,
                                            preserveScroll: true,
