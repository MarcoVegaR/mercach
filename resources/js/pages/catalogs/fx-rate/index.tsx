...
import { Coins, Database, Plus, RefreshCw } from 'lucide-react';
...
export default function IndexPage() {
    ...
    const [columnVisibility, setColumnVisibility] = React.useState<VisibilityState>({
        ...
        value_date: false,
        published_at: false,
        operational_from: false,
        operational_to: false,
        is_official: false,
        created_at: false,
    });
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('fx-rate_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.fx-rate.create'] || false,
        canEdit: auth?.can?.['catalogs.fx-rate.update'] || false,
        canDelete: auth?.can?.['catalogs.fx-rate.delete'] || false,
        canExport: auth?.can?.['catalogs.fx-rate.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.fx-rate.delete'] || false,
        canSetActive: auth?.can?.['catalogs.fx-rate.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.fx-rate.setActive'] || false,
    };
    ...
    const reloadData = React.useCallback(() => {
        ...
        router.get('/catalogs/fx-rate', params, {
            only: ['rows', 'meta'],
            preserveState: true,
            preserveScroll: true,
        });
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/fx-rate/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting],
    );
                            ...
                            <div className="flex items-center gap-2">
                                {permissions.canEdit && (
                                    <Button
                                        ...
                                        onClick={async () => {
                                            try {
                                                await new Promise<void>((resolve, reject) => {
                                                    router.post(
                                                        '/catalogs/fx-rate/sync',
                                                        {},
                                                        {
                                                            preserveState: false,
                                                            preserveScroll: true,
                                                            onSuccess: () => {
                                                                toast.success('Sincronización BCV ejecutada');
                                                                resolve();
                                                            },
                                                            onError: () => reject(new Error('sync_failed')),
                                                        },
                                                    );
                                                });
                                            } catch {
                                        ...
                                        title="Sincronizar última tasa desde BCV"
                                    >
                                ...
                                {permissions.canCreate && (
                                    <Link href="/catalogs/fx-rate/create">
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Tasas de cambio</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Tasa de cambio Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('fx-rate_table_density', d);
                                    }}
                                    stickyLeftColumnId="currency_code"
