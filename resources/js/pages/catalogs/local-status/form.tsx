...
export default function FormPage(props: PageProps) {
    ...
    function handleCancel() {
        router.visit('/catalogs/local-status', { preserveScroll: true });
    }
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (mode === 'create') {
            form.post(route('catalogs.local-status.store'));
        } else {
            const id = initial.id;
            ...
            form.put(route('catalogs.local-status.update', id));
        }
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={mode === 'edit' ? 'Editar Estado de local' : 'Crear Estado de local'} />
            <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
                <div className="py-8">
                    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
                        <h1 className="mb-4 text-2xl font-bold text-gray-900 dark:text-gray-100">
                            {mode === 'edit' ? 'Editar' : 'Crear'} Estado de local
                        </h1>
