...
import { ArrowLeft, Calendar, Hash, Pencil, Shield, Store, Trash2 } from 'lucide-react';
...
export default function ShowPage({ item: initialItem, meta: initialMeta, hasEditRoute, auth }: ShowProps) {
    const { item, meta, loading, activeTab, setActiveTab, loadPart } = useShow<Item>({
        endpoint: `/catalogs/local-status/${initialItem.id}`,
        initialItem,
        initialMeta,
    });
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Estado de local: ${String(item.name ?? item.code ?? item.id)}`} />
            ...
            <ShowLayout
                ...
                actions={
                    auth?.can && (auth.can['catalogs.local-status.update'] || auth.can['catalogs.local-status.delete']) ? (
                        <div className="flex gap-2">
                            {auth.can['catalogs.local-status.update'] && hasEditRoute && (
                                <Button onClick={() => router.visit(`/catalogs/local-status/${item.id}/edit`)}>
                                    <Pencil className="h-4 w-4" />
                                    Editar
                                </Button>
                            )}
                            {auth.can['catalogs.local-status.delete'] && (
                                <ConfirmAlert
                                    trigger={
                                    ...
                                    title="Eliminar Estado de Local"
                                    ...
                                    toastMessages={{
                                        ...
                                        success: 'Estado de local eliminado',
                                        error: 'No se pudo eliminar el estado de local',
                                    }}
                                />
                            )}
                        </div>
                                ...
                                <Card className="border-dashed">
                                    <CardContent className="pt-6">
                                        <div className="mx-auto max-w-prose py-8 text-center leading-relaxed">
                                            ...
                                            <p className="text-muted-foreground text-xs">Este estado no tiene locales asociados actualmente</p>
                                        </div>
                                    </CardContent>
                                </Card>
                            )}
