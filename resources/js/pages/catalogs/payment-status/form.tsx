...
export default function FormPage(props: PageProps) {
    ...
    function handleCancel() {
        router.visit('/catalogs/payment-status', { preserveScroll: true });
    }
    ...
    function handleSubmit(e: React.FormEvent) {
        ...
        if (mode === 'create') {
            clearUnsavedChanges();
            form.post(route('catalogs.payment-status.store'));
        } else {
            const id = initial.id;
            ...
            clearUnsavedChanges();
            form.put(route('catalogs.payment-status.update', id));
        }
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={mode === 'edit' ? 'Editar Estado de pago' : 'Crear Estado de pago'} />
            <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
                <div className="py-8">
                    <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
                        <h1 className="mb-4 text-2xl font-bold text-gray-900 dark:text-gray-100">
                            {mode === 'edit' ? 'Editar' : 'Crear'} Estado de pago
                        </h1>
