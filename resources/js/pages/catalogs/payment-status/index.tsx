...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('payment-status_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.payment-status.create'] || false,
        canEdit: auth?.can?.['catalogs.payment-status.update'] || false,
        canDelete: auth?.can?.['catalogs.payment-status.delete'] || false,
        canExport: auth?.can?.['catalogs.payment-status.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.payment-status.delete'] || false,
        canSetActive: auth?.can?.['catalogs.payment-status.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.payment-status.setActive'] || false,
    };
    ...
    const reloadData = React.useCallback(() => {
        ...
        router.get('/catalogs/payment-status', params, {
            only: ['rows', 'meta'],
            preserveState: true,
            preserveScroll: true,
        });
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/payment-status/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting],
    );
                ...
                <div className="py-8">
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="mb-8 flex items-center justify-between">
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Estados de pago</h1>
                            {permissions.canCreate && (
                                <Link href="/catalogs/payment-status/create">
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Estados de pago</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Estado de pago Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('payment-status_table_density', d);
                                    }}
