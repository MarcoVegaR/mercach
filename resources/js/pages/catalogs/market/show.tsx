...
import { ArrowLeft, Building, Calendar, Hash, MapPin, Pencil, Store, Trash2 } from 'lucide-react';
...
export default function ShowPage({ item: initialItem, meta: initialMeta, hasEditRoute, auth }: ShowProps) {
    const { item, meta, loading, activeTab, setActiveTab, loadPart } = useShow<Item>({
        endpoint: `/catalogs/market/${initialItem.id}`,
        initialItem,
        initialMeta,
    });
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Mercado: ${String(item.name ?? item.code ?? item.id)}`} />
            ...
            <ShowLayout
                ...
                actions={
                    auth?.can && (auth.can['catalogs.market.update'] || auth.can['catalogs.market.delete']) ? (
                        <div className="flex gap-2">
                            {auth.can['catalogs.market.update'] && hasEditRoute && (
                                <Button onClick={() => router.visit(`/catalogs/market/${item.id}/edit`)}>
                                    <Pencil className="h-4 w-4" />
                                    Editar
                                </Button>
                            )}
                            {auth.can['catalogs.market.delete'] && (
                                <ConfirmAlert
                                    trigger={
                                    ...
                                    toastMessages={{
                                        ...
                                        error: 'No se pudo eliminar el mercado',
                                    }}
                                />
                            )}
                        </div>
                            ...
                            <CardContent className="space-y-3">
                                <div className="flex items-center justify-between">
                                    <span className="text-muted-foreground flex items-center text-sm">
                                        <Building className="mr-1 inline h-4 w-4 text-green-500 dark:text-green-400" />
                                        Estado
                                    </span>
                                ...
                                <CardContent className="pt-6">
                                    <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                        ...
                                        <div>
                                            ...
                                            <dd className="mt-1 text-sm">{String(item.address ?? '—')}</dd>
                                        </div>
                                        <div>
                                ...
                                <Card className="border-dashed">
                                    <CardContent className="pt-6">
                                        <div className="mx-auto max-w-prose py-8 text-center leading-relaxed">
                                            ...
                                            <p className="text-muted-foreground text-xs">Este mercado no tiene locales asociados actualmente</p>
                                        </div>
                                    </CardContent>
                                </Card>
                            )}
