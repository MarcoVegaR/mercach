...
export default function IndexPage() {
    ...
    const [columnVisibility, setColumnVisibility] = React.useState<VisibilityState>({
        // Hide non-requested columns by default
        id: false,
        is_active: false,
        created_at: false,
    });
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('concessionaire_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.concessionaire.create'] || false,
        canEdit: auth?.can?.['catalogs.concessionaire.update'] || false,
        canDelete: auth?.can?.['catalogs.concessionaire.delete'] || false,
        canExport: auth?.can?.['catalogs.concessionaire.export'] || false,
    ...
    // Server-side filters
    const [filters, setFilters] = React.useState<ConcessionaireFilterValue>({});
    ...
    const reloadData = React.useCallback(() => {
        ...
        router.get('/catalogs/concessionaire', params, {
            only: ['rows', 'meta'],
            preserveState: true,
            preserveScroll: true,
        });
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/concessionaire/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting, filters],
    );
                ...
                <div className="py-8">
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="mb-8 flex items-center justify-between">
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Concesionarios</h1>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('concessionaire_table_density', d);
                                    }}
