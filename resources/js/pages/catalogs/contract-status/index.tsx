...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('contract-status_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.contract-status.create'] || false,
        canEdit: auth?.can?.['catalogs.contract-status.update'] || false,
        canDelete: auth?.can?.['catalogs.contract-status.delete'] || false,
        canExport: auth?.can?.['catalogs.contract-status.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.contract-status.delete'] || false,
        canSetActive: auth?.can?.['catalogs.contract-status.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.contract-status.setActive'] || false,
    };
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/contract-status/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting],
    );
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Estados de contrato" />
            <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
                <div className="py-8">
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        {/* Header with title and actions */}
                        <div className="mb-8 flex items-center justify-between">
                            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Estados de contrato</h1>
                            {permissions.canCreate && (
                                <Link href="/catalogs/contract-status/create">
                                    <Button className="flex items-center gap-2">
                                        <Plus className="h-4 w-4" />
                                        Nuevo Estado de contrato
                                    </Button>
                                </Link>
                            )}
                        </div>
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Estados de contrato</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Estado de contrato Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('contract-status_table_density', d);
                                    }}
