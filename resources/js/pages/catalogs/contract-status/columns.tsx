...
function ActionsCell({ row }: { row: Row }) {
    ...
    const canUpdate = !!auth?.can?.['catalogs.contract-status.update'];
    const canDelete = !!auth?.can?.['catalogs.contract-status.delete'];
    const canSetActive = !!auth?.can?.['catalogs.contract-status.setActive'];
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                        <Link href={`/catalogs/contract-status/${row.id}`} className="cursor-pointer">
                    ...
                    {canUpdate && (
                        <DropdownMenuItem asChild>
                            <Link href={`/catalogs/contract-status/${row.id}/edit`} className="cursor-pointer">
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.delete(`/catalogs/contract-status/${row.id}`, {
                            preserveState: false,
                            preserveScroll: true,
                            onSuccess: () => resolve(),
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.patch(
                            `/catalogs/contract-status/${row.id}/active`,
                            { active: !isActive },
                            {
                                preserveState: false,
                                preserveScroll: true,
