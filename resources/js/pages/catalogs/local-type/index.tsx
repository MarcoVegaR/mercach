...
import { Database, Plus, Shapes } from 'lucide-react';
...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('local-type_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.local-type.create'] || false,
        canEdit: auth?.can?.['catalogs.local-type.update'] || false,
        canDelete: auth?.can?.['catalogs.local-type.delete'] || false,
        canExport: auth?.can?.['catalogs.local-type.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.local-type.delete'] || false,
        canSetActive: auth?.can?.['catalogs.local-type.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.local-type.setActive'] || false,
    };
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/local-type/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting],
    );
                    ...
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="mb-8">
                            <IndexHeaderHero
                                ...
                                description="Catálogo de tipos de local"
                                actions={
                                    permissions.canCreate ? (
                                        <Link href="/catalogs/local-type/create">
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Tipos de local</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Tipo de local Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('local-type_table_density', d);
                                    }}
