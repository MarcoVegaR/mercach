...
import { Database, Plus, Tags } from 'lucide-react';
...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('market-tariff_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.market-tariff.create'] || false,
        canEdit: auth?.can?.['catalogs.market-tariff.update'] || false,
        canDelete: auth?.can?.['catalogs.market-tariff.delete'] || false,
        canExport: auth?.can?.['catalogs.market-tariff.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.market-tariff.delete'] || false,
        canSetActive: auth?.can?.['catalogs.market-tariff.setActive'] || false,
    ...
    const reloadWith = React.useCallback(
        (overrides?: Partial<{ pageIndex: number; pageSize: number; sorting: SortingState; globalFilter: string }>) => {
            const pIndex = overrides?.pageIndex ?? pageIndex;
            const pSize = overrides?.pageSize ?? pageSize;
            const srt = overrides?.sorting ?? sorting;
            const q = overrides?.globalFilter ?? globalFilter;
            ...
            if (srt.length > 0) {
                const s = srt[0];
                params.sort = String(s.id);
                params.dir = s.desc ? 'desc' : 'asc';
            }

            router.get('/catalogs/market-tariff', params, { preserveState: true, preserveScroll: true, replace: true });
        },
        [pageIndex, pageSize, sorting, globalFilter],
    );

    // Debounced search that also reloads data
    const debouncedSearch = React.useMemo(() => {
        ...
        return (value: string) => {
            ...
            timeoutId = setTimeout(() => {
                ...
                setPageIndex(0);
                reloadWith({ globalFilter: value, pageIndex: 0 });
            }, 300);
        };
    }, [reloadWith]);

    // Remove initial auto-reload; rely on handlers below
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/market-tariff/export?${usp.toString()}`;
        },
                    ...
                    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <div className="mb-8">
                            <IndexHeaderHero
                                ...
                                description="Defina la tarifa diaria por m² (EUR) vigente por mercado. Se usa para calcular los cargos de alquiler por m²."
                                actions={
                                    permissions.canCreate ? (
                                        <Link href="/catalogs/market-tariff/create">
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Tarifas de mercado</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Tarifa de mercado Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onPageChange={(i) => {
                                        setPageIndex(i);
                                        reloadWith({ pageIndex: i });
                                    }}
                                    onPageSizeChange={(size) => {
                                        setPageSize(size);
                                        setPageIndex(0);
                                        reloadWith({ pageSize: size, pageIndex: 0 });
                                    }}
                                    sorting={sorting}
                                    onSortingChange={(updater) => {
                                        const next = typeof updater === 'function' ? updater(sorting) : updater;
                                        setSorting(next);
                                        reloadWith({ sorting: next });
                                    }}
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('market-tariff_table_density', d);
                                    }}
                                    stickyLeftColumnId="price_per_m2_eur_minor"
