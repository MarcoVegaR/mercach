...
function ActionsCell({ row }: { row: Row }) {
    ...
    const canUpdate = !!auth?.can?.['catalogs.market-tariff.update'];
    const canDelete = !!auth?.can?.['catalogs.market-tariff.delete'];
    const canSetActive = !!auth?.can?.['catalogs.market-tariff.setActive'];
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                        <Link href={`/catalogs/market-tariff/${row.id}`} className="cursor-pointer">
                    ...
                    {canUpdate && (
                        <DropdownMenuItem asChild>
                            <Link href={`/catalogs/market-tariff/${row.id}/edit`} className="cursor-pointer">
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.delete(`/catalogs/market-tariff/${row.id}`, {
                            preserveState: false,
                            preserveScroll: true,
                            onSuccess: () => resolve(),
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.patch(
                            `/catalogs/market-tariff/${row.id}/active`,
                            { active: !isActive },
                            {
                                preserveState: false,
                                preserveScroll: true,
...
export const columns: ColumnDef<Row>[] = [
    ...
    {
        accessorKey: 'price_per_m2_eur_minor',
        header: 'Precio EUR/m² (día)',
        enableSorting: true,
        cell: ({ getValue }) => `€ ${formatMinorToMajor(getValue())}`,
    },
    { accessorKey: 'market_name', header: 'Mercado', enableSorting: true },
    { accessorKey: 'valid_from', header: 'Válido desde', enableSorting: true },
    {
