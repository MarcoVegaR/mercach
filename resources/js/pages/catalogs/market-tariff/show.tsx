...
export default function ShowPage() {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Tarifa de mercado: ${String((item as any).id)}`} />

            <ShowLayout
                header={
                    <div className="flex items-center gap-4">
                        <Link href="/catalogs/market-tariff" className="text-muted-foreground hover:text-foreground transition-colors">
                ...
                actions={
                    <div className="flex gap-2">
                        {hasEditRoute && (
                            <Button onClick={() => router.visit(`/catalogs/market-tariff/${item.id}/edit`)}>
                        ...
                        <ConfirmAlert
                            ...
                            onConfirm={async () => {
                                await new Promise<void>((resolve, reject) => {
                                    router.delete(`/catalogs/market-tariff/${item.id}`, {
                                        preserveState: false,
                                        preserveScroll: true,
                                        onSuccess: () => {
                                            resolve();
                                            router.visit('/catalogs/market-tariff');
                                        },
                    ...
                    <Card>
                        <CardContent className="pt-6">
                            <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                ...
                                <div>
                                    <dt className="text-muted-foreground text-sm font-medium">Válido desde</dt>
                                    <dd className="mt-1 text-sm">{String((item as any).valid_from ?? '—')}</dd>
                                </div>
                                <div>
                                    <dt className="text-muted-foreground text-sm font-medium">Precio EUR/m² (día)</dt>
                                    <dd className="mt-1 text-sm">€ {fmtMinor((item as any).price_per_m2_eur_minor)}</dd>
                                </div>
                                <div>
                                    <dt className="text-muted-foreground text-sm font-medium">Vigente</dt>
                                    <dd className="mt-1 text-sm">{(item as any).is_current ? 'Sí' : 'No'}</dd>
                                </div>
                                <div>
                            ...
                            <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <dd className="mt-1 text-sm">
                                        {formatDate(typeof (item as any).created_at === 'string' ? ((item as any).created_at as string) : null)}
                                    </dd>
                                </div>
                                <div>
                                    <dt className="text-muted-foreground text-sm font-medium">
                                    ...
                                    <dd className="mt-1 text-sm">
                                        {formatDate(typeof (item as any).updated_at === 'string' ? ((item as any).updated_at as string) : null)}
                                    </dd>
                                </div>
                            </dl>
                        </CardContent>
