...
export default function ShowPage() {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Local: ${String((item as any).name ?? (item as any).code ?? (item as any).id)}`} />

            <ShowLayout
                header={
                    <div className="flex items-center gap-4">
                        <Link href="/catalogs/local" className="text-muted-foreground hover:text-foreground transition-colors">
