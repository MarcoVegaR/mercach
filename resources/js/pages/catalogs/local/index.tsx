...
import { LocalFilters, type FilterOptions as LocalFilterOptions, type Filters as LocalFilterValue } from './filters';
...
export default function IndexPage() {
    ...
    const [density, setDensity] = React.useState<'comfortable' | 'compact'>(() => {
        ...
        const saved = window.localStorage.getItem('local_table_density');
        return saved === 'compact' ? 'compact' : 'comfortable';
    });

    const permissions = {
        canCreate: auth?.can?.['catalogs.local.create'] || false,
        canEdit: auth?.can?.['catalogs.local.update'] || false,
        canDelete: auth?.can?.['catalogs.local.delete'] || false,
        canExport: auth?.can?.['catalogs.local.export'] || false,
        canBulkDelete: auth?.can?.['catalogs.local.delete'] || false,
        canSetActive: auth?.can?.['catalogs.local.setActive'] || false,
        canBulkSetActive: auth?.can?.['catalogs.local.setActive'] || false,
    };
    ...
    const handleExport = React.useCallback(
        (format: string = 'csv') => {
            ...
            window.location.href = `/catalogs/local/export?${usp.toString()}`;
        },
        [pageIndex, pageSize, globalFilter, sorting, filters],
    );
                            ...
                            <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Total Locales</p>
                                ...
                                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-sm font-medium text-gray-600 dark:text-gray-400">Local Activos</p>
                        ...
                        <div className="overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-gray-200 dark:bg-gray-800 dark:ring-gray-700">
                            <div className="p-6">
                                <DataTable
                                    ...
                                    onDensityChange={(d) => {
                                        setDensity(d);
                                        if (typeof window !== 'undefined') window.localStorage.setItem('local_table_density', d);
                                    }}
                                    getRowId={(row) => String((row as unknown as { id?: number | string }).id ?? '')}
                                    stickyLeftColumnId="code"
                                />
                            </div>
                        </div>
