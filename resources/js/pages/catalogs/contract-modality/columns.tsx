...
function ActionsCell({ row }: { row: Row }) {
    ...
    const canUpdate = !!auth?.can?.['catalogs.contract-modality.update'];
    const canDelete = !!auth?.can?.['catalogs.contract-modality.delete'];
    const canSetActive = !!auth?.can?.['catalogs.contract-modality.setActive'];
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                        <Link href={`/catalogs/contract-modality/${row.id}`} className="cursor-pointer">
                    ...
                    {canUpdate && (
                        <DropdownMenuItem asChild>
                            <Link href={`/catalogs/contract-modality/${row.id}/edit`} className="cursor-pointer">
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.delete(`/catalogs/contract-modality/${row.id}`, {
                            preserveState: false,
                            preserveScroll: true,
                            onSuccess: () => resolve(),
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.patch(
                            `/catalogs/contract-modality/${row.id}/active`,
                            { active: !isActive },
                            {
                                preserveState: false,
                                preserveScroll: true,
