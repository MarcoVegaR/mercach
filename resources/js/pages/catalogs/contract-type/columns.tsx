...
function ActionsCell({ row }: { row: Row }) {
    ...
    const canUpdate = !!auth?.can?.['catalogs.contract-type.update'];
    const canDelete = !!auth?.can?.['catalogs.contract-type.delete'];
    const canSetActive = !!auth?.can?.['catalogs.contract-type.setActive'];
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem asChild>
                        <Link href={`/catalogs/contract-type/${row.id}`} className="cursor-pointer">
                    ...
                    {canUpdate && (
                        <DropdownMenuItem asChild>
                            <Link href={`/catalogs/contract-type/${row.id}/edit`} className="cursor-pointer">
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.delete(`/catalogs/contract-type/${row.id}`, {
                            preserveState: false,
                            preserveScroll: true,
                            onSuccess: () => resolve(),
            ...
            <ConfirmAlert
                ...
                onConfirm={async () => {
                    await new Promise<void>((resolve, reject) => {
                        router.patch(
                            `/catalogs/contract-type/${row.id}/active`,
                            { active: !isActive },
                            {
                                preserveState: false,
                                preserveScroll: true,
