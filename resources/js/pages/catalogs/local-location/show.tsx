...
import { ArrowLeft, Calendar, Hash, MapPin, Pencil, Store, Trash2 } from 'lucide-react';
...
export default function ShowPage({ item: initialItem, meta: initialMeta, hasEditRoute, auth }: ShowProps) {
    const { item, meta, loading, activeTab, setActiveTab, loadPart } = useShow<Item>({
        endpoint: `/catalogs/local-location/${initialItem.id}`,
        initialItem,
        initialMeta,
    });
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Ubicación de local: ${String(item.name ?? item.code ?? item.id)}`} />

            <ShowLayout
                header={
                    <div className="flex items-center gap-4">
                        <Link href="/catalogs/local-location" className="text-muted-foreground hover:text-foreground transition-colors">
                ...
                actions={
                    auth?.can && (auth.can['catalogs.local-location.update'] || auth.can['catalogs.local-location.delete']) ? (
                        <div className="flex gap-2">
                            {auth.can['catalogs.local-location.update'] && hasEditRoute && (
                                <Button onClick={() => router.visit(`/catalogs/local-location/${item.id}/edit`)}>
                                    <Pencil className="h-4 w-4" />
                                    Editar
                                </Button>
                            )}
                            {auth.can['catalogs.local-location.delete'] && (
                                <ConfirmAlert
                                    trigger={
                                    ...
                                    title="Eliminar Ubicación de Local"
                                    description={`¿Está seguro de eliminar "${String(item.name ?? item.code ?? item.id)}"? Esta acción no se puede deshacer.`}
                                    confirmLabel="Eliminar"
                                    onConfirm={async () => {
                                        await new Promise<void>((resolve, reject) => {
                                            router.delete(`/catalogs/local-location/${item.id}`, {
                                                preserveState: false,
                                                preserveScroll: true,
                                                onSuccess: () => {
                                                    resolve();
                                                    router.visit('/catalogs/local-location');
                                                },
                                    ...
                                    toastMessages={{
                                        ...
                                        success: 'Ubicación de local eliminada',
                                        error: 'No se pudo eliminar la ubicación de local',
                                    }}
                                />
                            )}
                        </div>
                            ...
                            <CardContent className="space-y-3">
                                <div className="flex items-center justify-between">
                                    <span className="text-muted-foreground flex items-center text-sm">
                                        <MapPin className="mr-1 inline h-4 w-4 text-red-500 dark:text-red-400" />
                                        Estado
                                    </span>
                                ...
                                {item.locals_count !== undefined && item.locals_count !== null && (
                                    <div className="flex items-center justify-between">
                                        <span className="text-muted-foreground flex items-center text-sm">
                                            <Store className="mr-1 inline h-4 w-4 text-blue-500 dark:text-blue-400" />
                                            Locales
                                        </span>
                                ...
                                <Card className="border-dashed">
                                    <CardContent className="pt-6">
                                        <div className="mx-auto max-w-prose py-8 text-center leading-relaxed">
                                            ...
                                            <p className="text-muted-foreground text-xs">Esta ubicación no tiene locales asociados actualmente</p>
                                        </div>
                                    </CardContent>
                                </Card>
                            )}
