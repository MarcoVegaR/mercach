...
class PaymentStatusUpdateRequest extends BaseUpdateRequest
{
    /**
    ...
    {
        $current = $this->route('payment_status');
        ...
        return [
            ...
            // 'code' => ['bail','required','string','max:50', Rule::unique('payment_statuses','code')->ignore($currentId)->withoutTrashed()],
            // 'name' => ['bail','required','string','max:120'],
            // 'is_active' => ['nullable','boolean'],
            // 'sort_order' => ['nullable','integer'],
            '_version' => ['nullable', 'string'],
            'code' => ['bail', 'required', 'string', 'min:2', 'max:30', 'regex:/^[A-Z0-9_\-\.]+$/', Rule::unique('payment_statuses', 'code')->where(fn ($q) => $q->whereRaw('UPPER(code) = ?', [strtoupper($this->input('code'))]))->ignore($currentId)->withoutTrashed()],
