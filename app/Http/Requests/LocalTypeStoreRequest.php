...
class LocalTypeStoreRequest extends BaseStoreRequest
{
    /**
    ...
    {
        return (bool) ($this->user()?->can('create', \App\Models\LocalType::class));
    }
    ...
    public function rules(): array
    {
        return [
            ...
            // 'code' => ['bail','required','string','max:50', Rule::unique('local_types','code')->withoutTrashed()],
            // 'name' => ['bail','required','string','max:120'],
            // 'is_active' => ['nullable','boolean'],
            // 'sort_order' => ['nullable','integer'],
            'code' => ['bail', 'required', 'string', 'min:2', 'max:50', 'regex:/^[A-Z0-9_\-\.]{2,50}$/', Rule::unique('local_types', 'code')->where(fn ($q) => $q->whereRaw('UPPER(code) = ?', [strtoupper($this->input('code'))]))->withoutTrashed()],
