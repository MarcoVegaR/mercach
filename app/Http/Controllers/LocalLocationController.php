<?php

declare(strict_types=1);

namespace App\Http\Controllers;

use App\Contracts\Services\LocalLocationServiceInterface;
use App\Http\Requests\LocalLocationIndexRequest;
use App\Http\Requests\LocalLocationStoreRequest;
use App\Http\Requests\LocalLocationUpdateRequest;
use App\Models\LocalLocation;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;
use Inertia\Inertia;

class LocalLocationController extends BaseIndexController
{
    use \App\Http\Controllers\Concerns\HandlesForm;

    private LocalLocationServiceInterface $serviceConcrete;

    public function __construct(LocalLocationServiceInterface $service)
    {
        parent::__construct($service);
        $this->serviceConcrete = $service;
    }

    protected function policyModel(): string
    {
        return \App\Models\LocalLocation::class;
    }

    protected function view(): string
    {
        return 'catalogs/local-location/index';
    }

    /**
     * Display a listing of the resource with extras injected.
     */
    public function index(Request $request): \Inertia\Response
    {
        $response = parent::index($request);

        // Inject stats (and other extras) from service
        $extras = $this->serviceConcrete->getIndexExtras();
        if (isset($extras['stats'])) {
            $response->with('stats', $extras['stats']);
        }

        // Expose whether the edit route exists so the UI can hide Edit buttons if missing
        $response->with('hasEditRoute', Route::has('catalogs.local-location.edit'));

        return $response;
    }

    protected function indexRequestClass(): string
    {
        return LocalLocationIndexRequest::class;
    }

    protected function indexRouteName(): string
    {
        return 'catalogs.local-location.index';
    }

    /**
     * Get route parameters for the model (override HandlesForm default to use snake param).
     *
     * @return array<string, mixed>
     */
    protected function getRouteParameters(Model $model): array
    {
        return ['local_location' => $model->getKey()];
    }

    protected function allowedExportFormats(): array
    {
        return ['csv', 'xlsx', 'json'];
    }

    protected function formView(string $mode): string
    {
        return 'catalogs/local-location/form';
    }

    protected function storeRequestClass(): string
    {
        return LocalLocationStoreRequest::class;
    }

    protected function updateRequestClass(): string
    {
        return LocalLocationUpdateRequest::class;
    }

    /**
     * Override export permission to match catalogs prefix (e.g., catalogs.tipo-documento.export).
     */
    protected function exportPermission(): string
    {
        return 'catalogs.local-location.export';
    }

    /**
     * @return array{code: string|null, name: string|null, is_active: bool|null}
     */
    protected function getEmptyModel(): array
    {
        return [
            'code' => null,
            'name' => null,
            'is_active' => null,
        ];
    }

    public function show(Request $request, LocalLocation $local_location): \Inertia\Response
    {
        $this->authorize('view', $local_location);

        $data = [
            'item' => $this->service->toItem($local_location),
            'hasEditRoute' => true,
        ];

        return Inertia::render('catalogs/local-location/show', $data);
    }

    public function setActive(Request $request, LocalLocation $local_location): \Illuminate\Http\RedirectResponse
    {
        $this->authorize('setActive', $local_location);
        $desired = (bool) $request->boolean('active');
        $local_location->setAttribute('is_active', $desired);
        $local_location->save();
        $actionText = $desired ? 'activado' : 'desactivado';

        return redirect()->route('catalogs.local-location.index')
            ->with('success', 'El registro ha sido '.$actionText.' correctamente.');
    }

    public function destroy(LocalLocation $local_location): \Illuminate\Http\RedirectResponse
    {
        $this->authorize('delete', $local_location);
        $this->service->delete($local_location);

        return redirect()->route('catalogs.local-location.index')
            ->with('success', 'Registro eliminado correctamente.');
    }
}
