...
class LocalLocationService extends BaseService implements LocalLocationServiceInterface
{
    ...
    {
        // Get locals count and data for this local location
        $localsCount = $model->getRelationValue('locals')
            ? $model->getRelationValue('locals')->count()
            : \App\Models\Local::query()->where('local_location_id', $model->getKey())->count();
        ...
        if ($locals) {
            ...
            $localsData = \App\Models\Local::query()
                ->where('local_location_id', $model->getKey())
                ->pluck('code')
                ->all();
        }
    ...
    public function toItem(Model $model): array
    {
        ...
        if ($localsCount === null) {
            $localsCount = $model->relationLoaded('locals')
                ? $model->getRelationValue('locals')->count()
                : (int) \App\Models\Local::query()->where('local_location_id', $model->getKey())->count();
        }
    ...
    public function loadShowData(Model $model, array $with = [], array $withCount = []): array
    {
        ...
        if (! empty($with)) {
            ...
            if (! empty($allowedWith)) {
                ...
                $model->load(['locals:id,local_location_id,code']);
    ...
    public function getIndexExtras(): array
    {
        ...
        $model = \App\Models\LocalLocation::query();
