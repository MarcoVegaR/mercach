...
class LocalTypeService extends BaseService implements LocalTypeServiceInterface
{
    ...
    {
        // Get locals count and data for this local type
        $localsCount = $model->getRelationValue('locals')
            ? $model->getRelationValue('locals')->count()
            : \App\Models\Local::query()->where('local_type_id', $model->getKey())->count();
        ...
        if ($locals) {
            ...
            $localsData = \App\Models\Local::query()
                ->where('local_type_id', $model->getKey())
                ->pluck('code')
                ->all();
        }
    ...
    public function toItem(Model $model): array
    {
        ...
        if ($localsCount === null) {
            $localsCount = $model->relationLoaded('locals')
                ? $model->getRelationValue('locals')->count()
                : (int) \App\Models\Local::query()->where('local_type_id', $model->getKey())->count();
        }
    ...
    public function loadShowData(Model $model, array $with = [], array $withCount = []): array
    {
        ...
        if (! empty($with)) {
            ...
            if (! empty($allowedWith)) {
                ...
                $model->load(['locals:id,local_type_id,code']);
