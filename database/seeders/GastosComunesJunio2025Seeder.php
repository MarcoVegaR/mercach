...
class GastosComunesJunio2025Seeder extends Seeder
{
    public function run(): void
    {
        ...
        $period = Carbon::create(2025, 6, 1)->startOfMonth();
        $periodStr = $period->toDateString();
        $issuedOn = $periodStr;
        $dueOn = $period->copy()->day(5)->toDateString();

        $usdPerM2Minor = $this->decimalToInt('2,16', 2);

        $csvPath = database_path('seeders/data/locales_solo_junio_2025.csv');
        if (! is_file($csvPath)) {
    ...
    private function syncRemovedLocals(int $marketId, string $periodStr, array $desiredLocalIds): void
    {
        ...
        foreach ($rows as $row) {
            ...
            $charges->cancel((int) $row->id, 'Removed from locales_solo_junio_2025.csv');
        }
    }
