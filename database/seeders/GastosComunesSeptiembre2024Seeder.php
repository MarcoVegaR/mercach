...
class GastosComunesSeptiembre2024Seeder extends Seeder
{
    public function run(): void
    {
        ...
        $period = Carbon::create(2024, 9, 1)->startOfMonth();
        $periodStr = $period->toDateString();
        $issuedOn = $periodStr;
        $dueOn = $period->copy()->day(5)->toDateString();

        $usdPerM2Minor = $this->decimalToInt('0,81', 2);

        $csvPath = database_path('seeders/data/locales_solo_septiembre.csv');
        if (! is_file($csvPath)) {
    ...
    private function syncRemovedLocals(int $marketId, string $periodStr, array $desiredLocalIds): void
    {
        ...
        foreach ($rows as $row) {
            ...
            $charges->cancel((int) $row->id, 'Removed from locales_solo_septiembre.csv');
        }
    }
